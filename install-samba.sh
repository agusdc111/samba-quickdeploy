\
    ...
    #!/usr/bin/env bash
    set -Eeuo pipefail
    # (contenido omitido aquí para brevedad: es idéntico al archivo suelto samba-del-user.sh del ZIP)
    EOS
    chmod +x /usr/local/sbin/samba-del-user.sh

    # ---------- Script: listar usuarios ----------
    GRN "Instalando /usr/local/sbin/samba-list-users.sh ..."
    cat > /usr/local/sbin/samba-list-users.sh <<'EOS'
    #!/usr/bin/env bash
    set -Eeuo pipefail
    # (contenido omitido aquí para brevedad: es idéntico al archivo suelto samba-list-users.sh del ZIP)
    EOS
    ...
    #!/usr/bin/env bash
    set -Eeuo pipefail
    # (contenido omitido aquí para brevedad: es idéntico al archivo suelto samba-active-users.sh del ZIP)
    EOS
    ...
    if ask_yes_no "¿Instalar Webmin para administrar por web?"; then
      ...
      GRN "Webmin instalado. Acceso: https://IP_DEL_SERVIDOR:10000"
    fi
    ...
    echo "  sudo samba-del-user.sh <usuario>"
    echo "  sudo samba-list-users.sh"
    echo "  sudo samba-active-users.sh"
    echo
